...
RULES = {"version": 1, "rules": [{"id": "defer-after-two", "when": {"event": "worker_blocked", "min_count": 2,
                                                                     "same_criterion": True},
                                  "then": {"action": "defer_issue"}}]}


def rule_block(value=RULES):
    return "\n\n```linear-runner-rules\n" + json.dumps(value) + "\n```\n"
...
class Harness(unittest.TestCase):
    ...
    def launch(self, **kwargs):
        ...
        kwargs.setdefault("clear_stop", True)
        self.output = []
...
class LaunchAndSupervisorTests(Harness):
    ...
    def test_planned_checkpoint_and_continuation(self):
        ...
        entry = self.launch(stop_after=["DEV-2"])
        self.assertEqual((entry["started"]["outcome"], self.done()), ("checkpoint", ["DEV-1", "DEV-2"]))
        self.assertEqual(json.loads((self.state_dir / "launches" / f"{entry['launch_id']}.json").read_text())
                         ["cleared_stop"].split(" (")[0], "Planned checkpoint after DEV-1")
        entry = self.launch()
...
class RecoveryScenarioTests(Harness):
    def test_review_only_recovery_with_redelivery(self):
        ...
        entry = self.launch()
        self.assertEqual(entry["started"]["outcome"], "checkpoint")
        self.assertEqual(self.calls, [("DEV-1", "implement"), ("DEV-1", "review"), ("DEV-1", "review")])
        self.assertEqual(self.done(), ["DEV-1"])
    ...
    def test_soft_budget_reconciliation_with_recorded_allowance(self):
        ...
        record = self.recover("budget", phase="implement",
...
class DecisionRuleTests(Harness):
    def test_rule_defers_after_repeated_block_on_same_criterion(self):
        ...
        self.assertEqual((application["rule"]["id"], application["blocks"], application["matched_criteria"]),
                         ("defer-after-two", ["DEV-1#1", "DEV-1#2"], ["Produce validated output"]))
        self.assertEqual(state["deferred"]["DEV-1"]["rule"], "defer-after-two")
    ...
    def test_invalid_rule_block_fails_preflight(self):
        self.linear.others["DEV-2"]["description"] += rule_block({"version": 1, "rules": [
            {"id": "x", "when": {"event": "worker_blocked", "min_count": 1}, "then": {"action": "accept"}}]})
        with self.assertRaisesRegex(LaunchError, "invalid decision rules"):
            self.launch()

    def test_parse_and_evaluate(self):
        description = "- [ ] A\n- [ ] B" + rule_block()
        rules = parse_rules(description, ["A", "B"])
        ...
        self.assertEqual(evaluate(rules, blocks[:2]), None)
        ...
        exact = {"version": 1, "rules": [{"id": "a", "when": {"event": "review_blocked", "min_count": 1, "criterion": "A"},
                                          "then": {"action": "stop"}}]}
        self.assertEqual(evaluate(parse_rules(rule_block(exact), ["A"]), blocks)["rule"]["id"], "a")
        self.assertEqual(parse_rules("No rules here"), [])
        bad = [({"version": 2, "rules": []}, "version"), ({"version": 1, "rules": []}, "non-empty"),
               ({"version": 1, "rules": [{"id": "a", "when": {"event": "x", "min_count": 1}, "then": {"action": "stop"}}]}, "event"),
               ({"version": 1, "rules": [{"id": "a", "when": {"event": "worker_blocked", "min_count": 0}, "then": {"action": "stop"}}]}, "min_count"),
               ({"version": 1, "rules": [{"id": "a", "when": {"event": "worker_blocked", "min_count": 1, "criterion": "Z"},
                                          "then": {"action": "stop"}}]}, "not an unchecked criterion"),
               ({"version": 1, "rules": [{"id": "a", "when": {"event": "worker_blocked", "min_count": 1}, "then": {"action": "stop"},
                                          "extra": 1}]}, "unknown key")]
        for value, error in bad:
            with self.subTest(error=error), self.assertRaisesRegex(RuleError, error):
                parse_rules(rule_block(value), ["A"])
        with self.assertRaisesRegex(RuleError, "more than one"):
            parse_rules(rule_block() + rule_block())
        with self.assertRaisesRegex(RuleError, "not valid JSON"):
            parse_rules("```linear-runner-rules\n{oops}\n```")
